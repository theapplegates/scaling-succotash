...
# Path to the openpgp crate, adjust if needed.
OPENPGP_DEP='sequoia-openpgp = { path = "../openpgp", version = "2.0.0", default-features = false, features = ["crypto-openssl"] }'

fix_package_section() {
  local f="$1"
  # Replace a bad name line with the correct one
  sed -i.bak -E 's|name = "sequoia-openpgp\s*=\s*\{.*\}|name = "sequoia-openpgp"|' "$f"
}

fix_openpgp_dependency() {
  local f="$1"
  # Remove any sequoia-openpgp dependency lines first, then append the correct one
  sed -i.bak '/sequoia-openpgp\s*=\s*{/d' "$f"
  # Append correct dependency to the [dependencies] section
  awk -v dep="$OPENPGP_DEP" '
    BEGIN {added=0}
    /^\[dependencies\]/ {print; if (!added) {print dep; added=1} next}
    {print}
  ' "$f" > "$f.tmp" && mv "$f.tmp" "$f"
}
...
remove_duplicate_dependencies_headers() {
  local f="$1"
  # Remove duplicate [dependencies] (keep the first one)
